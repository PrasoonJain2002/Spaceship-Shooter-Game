...
**				 BY
**
**	   Prasoon Jain(102016112), Chiranjeev Singh(102016016), Divyansh Kapoor(102016058)
**
*******************************************************************************************************************
*******************************************************************************************************************
...
#define MAX_STONES  100			//Maximum number of stones in a level
...
GLfloat ALienMouth[][2] = { {-6,21.5}, {-6.5,22}, {-9,21}, {-11,20.5}, {-20,20}, {-14,23}, {-9.5,28}, {-7,27}, {-6,26.5},
...
void initializeStoneArray() {
	...
	xStone[0] = -(200 * MAX_STONES) - 600;             //Spawn position for first stone appearance
...
void DrawAlienMouth()
{
	glColor3f(1, 1, 0);				//Mouth color
	glBegin(GL_POLYGON);
	for (i = 0; i <= 14; i++)
		glVertex2fv(ALienMouth[i]);
	glEnd();

	glColor3f(0, 0, 0);				//Mouth outline
	glBegin(GL_LINE_STRIP);
	for (i = 0; i <= 14; i++)
		glVertex2fv(ALienMouth[i]);
	glEnd();
...
void DrawLaserBeam() {
	...
	glColor3f(1, 0, 0);
	glBegin(GL_LINES);						// Drawing Laser
...
void DisplayHealthBar() {
	...
	sprintf_s(temp, "  HEALTH = %d", alienLife);
...
void startScreenDisplay()
{
	...
	const char *tmp = "Start Game";
	displayRasterText(-100, 340, 0.4, (char *)tmp);
	...
	if (mouseX >= -100 && mouseX <= 100 && mouseY >= 30 && mouseY <= 80) {
		...
		glColor3f(0, 0, 0);
	const char* tmp1 = "Instructions";
	displayRasterText(-120, 80, 0.4, (char *)tmp1);
	...
	if (mouseX >= -100 && mouseX <= 100 && mouseY >= -90 && mouseY <= -40) {
		...
		glColor3f(0, 0, 0);
	const char* tmp2 = "Quit";
	displayRasterText(-100, -170, 0.4, (char *)tmp2);
...
void GameOverScreen()
{
	...
	glColor3f(0, 0, 1);
	const char* tmp3 = "G A M E    O V E R ! ! !";
	displayRasterText(-200, 640, 0.4, (char *)tmp3);
	glColor3f(0, 0, 0);
	char temp[40];
	...
	if (highScore <= Score) {
		...
		sprintf_s(temp2, "Highest Score :%d", highScore);
	...
	if (mouseX >= -100 && mouseX <= 100 && mouseY >= 25 && mouseY <= 75) {
		...
		glColor3f(0, 0, 0);
	const char* tmp4= "Restart";
	displayRasterText(-70, 80, 0.4, (char *)tmp4);
	...
	if (mouseX >= -100 && mouseX <= 100 && mouseY >= -100 && mouseY <= -50) {
		...
		glColor3f(0, 0, 0);
	const char* tmp5 = "Quit";
	displayRasterText(-100, -170, 0.4, (char *)tmp5);
...
void backButton() {
	if (mouseX <= -450 && mouseX >= -500 && mouseY >= -275 && mouseY <= -250) {
		glColor3f(0, 0, 1);					//Change the color of the button
		if (mButtonPressed) {
			mButtonPressed = false;
			instructionsGame = false;
			startScreenDisplay();
...
void InstructionsScreenDisplay()
{
	...
	glLineWidth(50);
	glBegin(GL_LINE_LOOP);               //Border
	glVertex3f(-1000, -400, 0.5);
	glVertex3f(-1000, 520, 0.5);
	glVertex3f(750, 520, 0.5);
	glVertex3f(750, -400, 0.5);
	glEnd();
	glColor3f(1, 1, 0);
	...
	displayRasterText(-900, -200, 0.4, (char*)"You Get 1 point for shooting each object and 50 points for completing each level ");
...
void display()					//Display all screens according to the boolean values
{
